...
### --- HeLa whole cell (6 replicates) -----------------------------------------
rep_cols_wc <- paste0("HeLaWCuntagged", 1:6)

wc_long <- HeLaWCIP_df %>% 
  select(ClassKey, LipidMolec, all_of(rep_cols_wc)) %>% 
  pivot_longer(all_of(rep_cols_wc), names_to = "Rep", values_to = "Abund")
...
ggplot() +
  geom_col(data = wc_stats,
           aes(x = ClassKey, y = mean_abund),
           fill = "grey70", width = 0.7) +
  geom_errorbar(data = wc_stats,
                aes(x = ClassKey,
                    ymin = mean_abund - sd_abund,
                    ymax = mean_abund + sd_abund),
                width = 0.25) +
  geom_jitter(data = wc_long,
              aes(x = ClassKey, y = Abund),
              alpha = 0.3, width = 0.25, size = 1) +
  scale_x_discrete(limits = c("Cer","ChE","CL","DG","DGDG","FA","GD1","GM1","GM2",
                              "GM3","GT1a","Hex1Cer","Hex2Cer","Hex3Cer",
                              "LBPA / BMP","LPC","LPE","PA","PC","PC-P","PE",
                              "PG","PI","PS","SM","SPH","ST","TG")) +
  labs(x = "Lipid class", y = "Abundance") +
  theme_classic() +
