...
```{r}
# --- 1. Add genotype and neurontype columns by parsing `file`
...
# --- 2. Aggregate per genotype × neurontype
agg_df <- th_summary %>%
  group_by(genotype, neurontype) %>%
  summarise(
    PercentPos = mean(percent_th_positive, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  mutate(PercentNeg = 100 - PercentPos)

# --- 3. Pivot longer for plotting
stacked_df <- agg_df %>%
  pivot_longer(
    cols = c(PercentPos, PercentNeg),
    names_to = "THstatus",
    values_to = "Percent"
  ) %>%
  mutate(
    THstatus = ifelse(THstatus == "PercentPos", "TH+", "TH−"),
