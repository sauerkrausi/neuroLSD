...
```{r}
# --- 2. Filter for genotypes of interest
genotypes_keep <- c("Ctrl", "ASAH1", "SMPD1")

stacked_df <- th_summary %>%
  filter(genotype %in% genotypes_keep) %>%
  mutate(
    PercentNeg = 100 - percent_th
  ) %>%
  pivot_longer(
    cols = c(percent_th, PercentNeg),
    names_to = "THstatus",
    values_to = "Percent"
  ) %>%
  mutate(
    THstatus = ifelse(THstatus == "percent_th", "TH+", "TH−"),
    Group = paste(genotype, THstatus, sep = "_"),
    THstatus = factor(THstatus, levels = c("TH+", "TH−")),
    genotype = factor(genotype, levels = genotypes_keep)
  )

# --- 3. Compute label positions
stacked_df <- stacked_df %>%
  group_by(genotype, neurontype) %>%
  arrange(desc(THstatus)) %>%   # so TH+ is stacked on top
  mutate(
...
th_stacked_plot <- ggplot(stacked_df, aes(x = genotype, y = Percent, fill = Group)) +
  ...
  scale_y_continuous(limits = c(0, 100), expand = c(0, 0)) +  # force 0–100
...
ggsave(filename = file.path(theval_outdir, "TH_positive_vs_negative_stacked.pdf"),
       plot = th_stacked_plot, width = 6, height = 4, device = cairo_pdf)
