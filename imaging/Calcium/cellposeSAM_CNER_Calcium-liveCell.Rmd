...
base_colors <- RColorBrewer::brewer.pal(length(geno_levels), "Paired")  
...
for (sample_id in all_sample_ids) {
  ...
  df_sample <- df_sample %>%
    ...
    dplyr::mutate(
      ...
      dF_F_binary = ifelse(dF_F_binary == 1 & (run_len < 3 | run_len > 30), 0, dF_F_binary)
    ) %>%
    dplyr::ungroup()
  ...
  firing_stats <- df_sample %>%
    ...
    dplyr::summarise(
      ...
      spike_amplitude = if (sum(dF_F_binary, na.rm = TRUE) > 0) {
        quantile(dF_F_scaled[dF_F_binary == 1], 0.75, na.rm = TRUE)
        } else {
         NA_real_
      }
    ) %>%
    dplyr::mutate(
      Sample = sample_id,
      firing_rate_per_min = firing_events / (max(df_sample$ImageNumber) / (frame_rate * 60)),
      Group = group_label
    )

  # save firing stats in events_list   
  ...
  sample_summary <- firing_stats %>%
    dplyr::summarise(
      ...
      Mean_percent_active_frames_norm = Mean_percent_active_frames  # already normalized
    )
...
# ----------------------------
# Extract first file per genotype
# ----------------------------
first_files_per_genotype <- traces_with_genotype %>%
  distinct(file, Genotype) %>%
  group_by(Genotype) %>%
  slice(1) %>%
  pull(file)

first_trace_df <- traces_with_genotype %>%
  filter(file %in% first_files_per_genotype)

# Plot Example Traces per Genotype
ggplot(first_trace_df, aes(x = ImageNumber, y = dF_F_scaled, group = ObjectNumber)) +
  geom_line(alpha = 0.5, size = 0.3) +
  facet_wrap(~Genotype, scales = "free_y") +
  labs(title = "Example dF/F Traces (First File per Genotype)",
       x = "Frame", y = "dF/F (scaled)") +
  theme_bw(base_size = 6)

ggsave(file.path(ca_results_dir, "example_traces_first_file_per_genotype.pdf"),
