...
```{r}
# ----------------------------------------------------------------------
# Updated plot_metric(): Non-parametric version using Kruskal-Wallis + Wilcoxon
# ----------------------------------------------------------------------
plot_metric <- function(dat, metric, ylab, outfile, base_dir) {
  ...
  # --- Kruskal-Wallis test
  kruskal_pval <- kruskal.test(as.formula(paste0(metric, " ~ Genotype")), data = dat)$p.value

  # --- Pairwise Wilcoxon (Mann-Whitney) tests
  filtered_dat <- dplyr::filter(dat, !is.na(!!metric_sym))
  pw <- pairwise.wilcox.test(filtered_dat[[metric]], filtered_dat$Genotype, p.adjust.method = "holm")
  ...
  if (!is.null(pw$p.value)) {
    ...
    pairwise_df <- tibble(group1 = character(0), group2 = character(0),
                          p.value = numeric(0), p.adj = character(0), significance = character(0),
  ...
  list(
    plot = p,
    pval = bind_rows(
      tibble(Metric = metric, group1 = "KruskalWallis", group2 = "", p.value = kruskal_pval),
      pairwise_df %>% mutate(Metric = metric) %>%
        select(Metric, group1, group2, p.value, significance)
    )
  )
}
...
## IIId. Community Size Analysis (Non-parametric) 
```{r}
# ---------------------------------------------------------------------------
# Community Size Analysis (Non-parametric) 
# ---------------------------------------------------------------------------
comm_df <- add_genotype(comm_df, genotype_map) %>%
...
comm_kw <- kruskal.test(CommunitySize ~ Genotype, data = comm_df)
comm_kw_pval <- comm_kw$p.value

# --- Pairwise Wilcoxon tests
comm_pw <- pairwise.wilcox.test(comm_df$CommunitySize, comm_df$Genotype, p.adjust.method = "holm")
...
bin_size <- floor(baseline_window * 0.2)
...
calc_nonparametric_stats <- function(df, metric_col, group_col = "Genotype") {
  ...
  colnames(pairwise_df) <- c("group1", "group2", "p.value")
  pairwise_df <- pairwise_df %>%
    dplyr::mutate(Metric = metric_col) %>%
    dplyr::select(Metric, group1, group2, p.value)

  all_pvals <- dplyr::bind_rows(
...
# ---------------------------------------------------------------------------
all_pvals <- dplyr::bind_rows(
  calc_nonparametric_stats(events_df, "firing_rate_per_min"),
  calc_nonparametric_stats(events_df, "percent_active"),
  calc_nonparametric_stats(events_df, "spike_amplitude"),
  calc_nonparametric_stats(comm_df, "CommunitySize")
)

# Save updated p-value summary
write.csv(all_pvals,
