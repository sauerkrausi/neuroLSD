...
parse_metadata <- function(file_list) {
  tibble(File = file_list) %>%
    mutate(
      ...
      Genotype  = parts[,3],
      Treatment = parts[,4],
      Replicate = ifelse(ncol(parts) >= 5, parts[,5], NA),
      Sample    = str_c(Celltype, Timepoint, Genotype, Treatment, sep = "_")
    ) %>%
    select(File, Sample, Celltype, Timepoint, Genotype, Treatment, Replicate)
}
...
# -----------------------------
process_pair <- function(sample_name, intensity_df, centroids_df) {
  intensity_sub <- intensity_df %>% filter(Sample == sample_name)
  centroids_sub <- centroids_df %>% filter(Sample == sample_name)
  ...
  intensity_matrix <- intensity_sub %>%
    select(where(is.numeric), -time)  # drop time, keep only numeric ROI cols
  ...
  intensity_filtered <- intensity_sub %>%
    mutate(across(where(is.numeric) & !matches("time"),
                  ~pmax(.x, filter_thresh)))

  # --- Calculate dF/F
  traces_dff <- intensity_filtered %>%
    mutate(across(where(is.numeric) & !matches("time"),
                  ~calculate_dff(.x, window = baseline_window, smoothing_k = smoothing_k)))

  # --- Reshape to long format
  traces_long <- traces_dff %>%
    mutate(ImageNumber = row_number()) %>%
    pivot_longer(
      cols = where(is.numeric) & !matches("time|ImageNumber"),
      names_to = "ObjectNumber",
      values_to = "Intensity_IntegratedIntensity_Fura",
      values_drop_na = TRUE
    ) %>%
    mutate(ObjectNumber = as.integer(ObjectNumber)) %>%
    left_join(centroids_sub, by = c("ObjectNumber", "Sample"))
...
# -----------------------------
dff_results <- map(unique(intensity_df$Sample), ~process_pair(.x, intensity_df, centroids_df)) %>%
  compact() %>%
  bind_rows()
