...
process_pair <- function(file_id, intensity_df, centroids_df) {
  ...
  metadata <- distinct(intensity_sub, FileID, Sample, Celltype, Timepoint, Genotype, Treatment, Replicate)

  traces_long <- traces_long %>%
    left_join(metadata, by = "FileID") %>%
    select(FileID, Sample, Celltype, Timepoint, Genotype, Treatment, Replicate,
           frame, ImageNumber, ObjectNumber,
           Intensity_IntegratedIntensity_Fura, centroid_x, centroid_y)
...
for (sample_id in all_sample_ids) {
  ...
  # --- dF/F computation
  df_sample <- dplyr::filter(dff_results, FileID == sample_id) %>%
    dplyr::select(-matches("^centroid_")) %>%
    dplyr::left_join(centroid_df_clean, by = c("FileID", "ObjectNumber")) %>%
...
# Add is_high_corr to centroids_df
...
centroids_df <- left_join(centroids_df, corr_annot,
                          by = c("FileID" = "Sample", "ObjectNumber" = "Label"))

# Ensure ObjectNumber has matching types
dff_results  <- dff_results  %>% mutate(ObjectNumber = as.character(ObjectNumber))
centroids_df  <- centroids_df  %>% mutate(ObjectNumber = as.character(ObjectNumber))

# Add genotype, bin, and join metadata
traces_with_genotype <- dff_results %>%
  left_join(centroids_df, by = c("FileID" = "file", "ObjectNumber")) %>%
  mutate(
