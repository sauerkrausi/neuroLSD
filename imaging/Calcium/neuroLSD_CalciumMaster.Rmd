...
# --- Save plot
ggsave(filename = file.path(camaster_plot_outdir, "Active_frames_iDAonly.pdf"), plot = perc_active_iDA_plot, width = 2, height = 6, device = cairo_pdf)
...
active_df_iDA_norm <- active_df_iDA_norm %>%
  ...
  mutate(
    ...
    Genotype = factor(Genotype, levels = c("Ctrl", "ASAH1")),
    Treatment = factor(Treatment, levels = c("Fed_Fluo4", "KCL_Fluo4")),
    Timepoint = factor(Timepoint, levels = c("d50", "d60"))
  )

# Optional: restrict to d50 only until d60 data are in
active_df_iDA_norm <- active_df_iDA_norm %>%
  filter(Timepoint == "d50")
...
# --- Save plot
ggsave(filename = file.path(camaster_plot_outdir, "Active_frames_normCtrlTime_iDAonly.pdf"), plot = active_only_iDA_plot, width = 2, height = 5, device = cairo_pdf)
...
# --- Save plot
ggsave(filename = file.path(camaster_plot_outdir, "Active_frames_total_iDAonly.pdf"), plot = perc_active_total_iDA_plot, width = 2, height = 6, device = cairo_pdf)
...
ida_summary_df <- master_combined_sample_level_normalised_metrics %>%
  filter(
    Celltype == "iDA",
    Treatment %in% c("Fed_Fluo4", "KCL_Fluo4"),
    Timepoint %in% c("d50") # Extend to d60 later
  ) %>%
  group_by(Genotype, Timepoint, Treatment) %>%
  summarise(
    FiringEventsPerMin = mean(Firing_events_per_min, na.rm = TRUE),
...
# --- Save
ggsave(file.path(camaster_plot_outdir, "Firing_events_iDA.pdf"), plot = ida_firing_raw_plot, width = 2, height = 4, device = cairo_pdf)
ggsave(file.path(camaster_plot_outdir, "Firing_events_iDA_norm.pdf"), plot = ida_firing_norm_plot, width = 2, height = 4, device = cairo_pdf)
```
