...
within_treatment_stats <- stats_df %>%
  ...
  group_modify(~ {
    ...
    bind_rows(lapply(comp_pairs, function(comp) {
      bind_rows(
        safe_run_pairwise_test(.x, "Mean_percent_active_frames", "Treatment", comp[1], comp[2]),
        safe_run_pairwise_test(.x, "Firing_events_per_min", "Treatment", comp[1], comp[2]),
        safe_run_pairwise_test(.x, "Percent_cells_active", "Treatment", comp[1], comp[2])
      ) %>% mutate(
...
across_time_stats <- stats_df %>%
  ...
  group_modify(~ {
    ...
    bind_rows(lapply(comp_pairs, function(comp) {
      bind_rows(
        safe_run_pairwise_test(.x, "Mean_percent_active_frames", "Timepoint", comp[1], comp[2]),
        safe_run_pairwise_test(.x, "Firing_events_per_min", "Timepoint", comp[1], comp[2]),
        safe_run_pairwise_test(.x, "Percent_cells_active", "Timepoint", comp[1], comp[2])
      ) %>% mutate(
...
between_genotype_stats <- stats_df %>%
  ...
  group_modify(~ {
    ...
    bind_rows(
      safe_run_pairwise_test(.x, "Mean_percent_active_frames", "Genotype", "Ctrl", "ASAH1"),
      safe_run_pairwise_test(.x, "Firing_events_per_min", "Genotype", "Ctrl", "ASAH1"),
      safe_run_pairwise_test(.x, "Percent_cells_active", "Genotype", "Ctrl", "ASAH1")
    ) %>% mutate(
...
# --- Label test types and adjust p-values
all_stats <- all_stats %>%
  mutate(
    TestType = case_when(
