...
community_plot_df <- master_combined_cell_community_membership %>%
  filter(
    ...
    CommunitySize >= 3       # filter out small communities
  ) %>%
  mutate(
...
# --- Save plot
ggsave(filename = file.path(camaster_plot_outdir, "Percent_Active_iNonly.pdf"), plot = perc_active_plot, width = 6, height = 6, device = cairo_pdf)
...
perc_active_total_plot <- ggplot(stacked_total_df, aes(x = BarGroup, y = Percent, fill = Group)) +
  ...
  facet_wrap(~ Genotype, nrow = 1) +
  labs(
    title = "Percent Active vs Non-Active Frames (iN only)",
    y = "Percent of Frames (%)",
    x = "Timepoint & Treatment"
  ) +
  scale_fill_manual(values = group_colors) +
...
# --- Save plot
ggsave(filename = file.path(camaster_plot_outdir, "total Percent_Active_iNonly.pdf"), plot = perc_active_total_plot, width = 6, height = 6, device = cairo_pdf)
```
...
# --- Save plot
ggsave(filename = file.path(camaster_plot_outdir, "Percent_Active_iDAonly.pdf"), plot = perc_active_iDA_plot, width = 6, height = 6, device = cairo_pdf)
...
ggsave(filename = file.path(camaster_plot_outdir, "Firing_events_normtime.pdf"),plot = firing_events_norm_time_plot, width = 4, height = 4, device = cairo_pdf)
