...
# -------------------
timepoint_dirs <- list(
  d33iDA = "/Users/felix/Documents/PostDoc/Harvard/03_LSD-PD/Microscopy/20241127_diff129_iN-iDA_Ctrl-ASAH1_Calcium-liveCell/cpsamWF/nd2_labels/calcium_analysis_results_dataframes",
  d38 = "/Users/felix/Documents/PostDoc/Harvard/03_LSD-PD/Microscopy/20250710_diff138_iN_d38_Ctrl-ASAH1_Flou4/cpsam_d38/nd2_labels/calcium_analysis_results_dataframes",
  d42 = "/Users/felix/Documents/PostDoc/Harvard/03_LSD-PD/Microscopy/20250714_diff138_iN_d42_Ctrl-ASAH1_Flou4/nd2_labels/calcium_analysis_results_dataframes",
  d45 = "/Users/felix/Documents/PostDoc/Harvard/03_LSD-PD/Microscopy/20250717_diff138_iN_d45_Ctrl-ASAH1_Flou4/nd2_labels/calcium_analysis_results_dataframes",
  d50 = "/Users/felix/Documents/PostDoc/Harvard/03_LSD-PD/Microscopy/20250721_diff138_iN_d50_Ctrl-ASAH1_Flou4/nd2iN_labels/calcium_analysis_results_dataframes",
  d50iDA = "/Users/felix/Documents/PostDoc/Harvard/03_LSD-PD/Microscopy/20250721_diff138_iN_d50_Ctrl-ASAH1_Flou4/nd2iDA_labels/calcium_analysis_results_dataframes",
  d60 = "/Users/felix/Documents/PostDoc/Harvard/03_LSD-PD/Microscopy/20250731_diff138_iN_d60_Ctrl-ASAH1_Flou4/nd2iN_labels/calcium_analysis_results_dataframes",
  d60iDA = "/Users/felix/Documents/PostDoc/Harvard/03_LSD-PD/Microscopy/20250731_diff138_iN_d60_Ctrl-ASAH1_Flou4/nd2iDA_labels/calcium_analysis_results_dataframes"
)
...
# DEFINE OUTPUT DIR  
# -------------------
camaster_outdir <- "/Users/felix/Documents/PostDoc/Harvard/03_LSD-PD/Microscopy/camaster_outdir"
dir.create(camaster_outdir, showWarnings = FALSE, recursive = TRUE)

camaster_plot_outdir <- "/Users/felix/Documents/PostDoc/Harvard/03_LSD-PD/Microscopy/camaster_outdir/plots"
...
# IV. QC plots Ctrl basal-sitmulation-inhibition
```{r}
# -----------------------------
# QC plots 
# -----------------------------
...
active_df_norm <- active_df_norm %>%
  ...
  mutate(
    ...
    Timepoint = factor(Timepoint, levels = c("d38", "d45", "d50"))
  )
...
```{r}
# -----------------------------
# Stats: Active Frames, Firing Events, Total Active ROIs
...
# --- Safe wrapper to always return tibble
safe_run_pairwise_test <- function(df, metric_col, group_col, group1, group2) {
  res <- run_pairwise_test(df, metric_col, group_col, group1, group2)
...
# Define metrics (raw names used internally)
metrics_to_test <- c("Mean_percent_active_frames",
                     "Firing_events_per_min",
                     "Percent_cells_active")
...
within_treatment_stats <- stats_df %>%
  ...
  group_modify(~ {
    ...
    bind_rows(lapply(comp_pairs, function(comp) {
      bind_rows(lapply(metrics_to_test, function(met) {
        safe_run_pairwise_test(.x, met, "Treatment", comp[1], comp[2])
      })) %>% mutate(
        ComparisonType = "WithinGenotype_Treatment",
...
# --- 2. Within-genotype across timepoints
across_time_stats <- stats_df %>%
  group_by(Celltype, Genotype, Treatment) %>%
  group_modify(~ {
    ...
    bind_rows(lapply(comp_pairs, function(comp) {
      bind_rows(lapply(metrics_to_test, function(met) {
        safe_run_pairwise_test(.x, met, "Timepoint", comp[1], comp[2])
      })) %>% mutate(
...
between_genotype_stats <- stats_df %>%
  ...
  group_modify(~ {
    if (n_distinct(.x$Genotype) < 2) return(tibble())
    bind_rows(lapply(metrics_to_test, function(met) {
      safe_run_pairwise_test(.x, met, "Genotype", "Ctrl", "ASAH1")
    })) %>% mutate(
      ComparisonType = "BetweenGenotype",
      Treatment = unique(.x$Treatment),
      Timepoint = unique(.x$Timepoint),
      Celltype = unique(.x$Celltype)
    )
...
# --- Replace Metric names with clean labels and adjust p-values
all_stats <- all_stats %>%
  mutate(
    Metric = case_when(
      Metric == "Mean_percent_active_frames" ~ "ActiveFrames",
      Metric == "Firing_events_per_min"      ~ "FiringEvents",
      Metric == "Percent_cells_active"       ~ "TotalActiveROIs",
      TRUE ~ Metric
    ),
    TestType = Metric,  # keep Metric and TestType consistent
