...
base_path <- "/Users/felix/Documents/PostDoc/Harvard/03_LSD-PD/Microscopy/20250710_diff138_iN_d38_Ctrl-ASAH1_Flou4/cpsam_d38/test"
...
for (sample_id in all_sample_ids) {
  ...
  # Save to correlation list
  cor_list[[clean_sample_id]] <- cor_mat
  ...
  ##  Correlation matrix of raw dF/F traces
  cor_mat_raw <- tryCatch({
    cm <- cor(t(dff_mat), use = "pairwise.complete.obs")
    cm[!is.finite(cm)] <- 0
    cm
  }, error = function(e) {
    warning(paste("Raw trace correlation failed for", clean_sample_id, ":", e$message))
    matrix(0, nrow = nrow(dff_mat), ncol = nrow(dff_mat),
           dimnames = list(df_sample$ObjectNumber, df_sample$ObjectNumber))
  })
  
  # Optionally store in another list
  cor_raw_list[[clean_sample_id]] <- cor_mat_raw
  
  # Plot heatmap
  pdf(file.path(output_dir, paste0(clean_sample_id, "_RawTrace_CorrelationMatrix.pdf")), width = 6, height = 6)
  tryCatch({
    pheatmap(cor_mat_raw, clustering_method = "complete",
             main = "Raw Trace Correlation Matrix", border_color = NA, fontsize = 6)
  }, error = function(e) {
    message("Skipped raw trace heatmap for ", clean_sample_id, ": ", e$message)
  })
  dev.off()
...
summary_df <- bind_with_sample(summary_list)
...
summary_df <- annotate_sample_metadata(summary_df)
```
...
```{r}
# ------------------------------------------------------
# Convert each corr  matrix to long format and then bind
# ------------------------------------------------------
cor_list_long_df <- imap_dfr(cor_list, function(mat, sample_id) {
  as.data.frame(mat) %>%
    rownames_to_column("ROI1") %>%
    pivot_longer(-ROI1, names_to = "ROI2", values_to = "Correlation") %>%
    mutate(SampleID = sample_id)
})

cor_raw_long_df <- imap_dfr(cor_raw_list, function(mat, sample_id) {
  as.data.frame(mat) %>%
    rownames_to_column("ROI1") %>%
    pivot_longer(-ROI1, names_to = "ROI2", values_to = "Correlation") %>%
    mutate(SampleID = sample_id)
...
# ------------------------------------------------------
write.csv(cor_list_long_df, file.path(ca_results_dataframes, "combined_cell_pairwise_correlations.csv"), row.names = FALSE)
write.csv(cor_raw_long_df, file.path(ca_results_dataframes, "combined_cell_pairwise_correlations_traces.csv"), row.names = FALSE)
