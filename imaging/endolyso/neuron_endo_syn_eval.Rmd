...
```{r}
# Color map kept here for later plotting
group_colors <- c(
  "Ctrl_iN"   = "grey60",
  "Ctrl_iDA"  = "grey80",
  "ASAH1_iN"  = "dodgerblue3",
...
# ---- User inputs: replace the URLs with your real links or local paths ----
eval_input_tbl <- tibble::tibble(
  file_url = c(
    "https://example.org/control_day30.csv",
    "https://example.org/asah1_day30.csv",
    "https://example.org/control_day38.csv",
    "https://example.org/asah1_day38.csv"
  ),
  genotype = c("Ctrl", "ASAH1", "Ctrl", "ASAH1"),
  day      = c(30, 30, 38, 38)
)

# ---- Output path ----
eval_output_dir  <- "/Users/felix/Documents/PostDoc/Harvard/03_LSD-PD/Microscopy/20241225_diff129_iN-iDA_d23-d30-d38/combined_eval"
eval_output_file <- file.path(eval_output_dir, "combined_per_image_eval.csv")

# ---- Helper: robust reader that preserves columns as best as possible ----
eval_read_one <- function(file_path_or_url) {
  readr::read_csv(
...
# ---- Main: read, annotate, harmonize, write ----
eval_combine_and_save <- function(input_tbl,
                                  out_file,
                                  out_dir = dirname(out_file)) {
  if (!dir.exists(out_dir)) dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

  safe_read <- purrr::possibly(eval_read_one, otherwise = NULL)

  annotated_list <- purrr::pmap(
    list(input_tbl$file_url, input_tbl$genotype, input_tbl$day),
    function(u, g, d) {
      df <- safe_read(u)
      ...
      df %>%
        mutate(
          genotype = g,
          day = as.integer(d),
          source_file = as.character(u)
        )
    }
  )

  non_null <- annotated_list[!vapply(annotated_list, is.null, logical(1))]
  if (length(non_null) == 0L) {
    stop("No files could be read. Check the URLs or paths.")
  }

  # bind_rows will union columns across files; missing columns become NA
  eval_combined_df <- dplyr::bind_rows(non_null)

  # Optional: enforce a sane column order if common columns exist
  ...
  eval_combined_df <- eval_combined_df[, ordered_cols, drop = FALSE]

  readr::write_csv(eval_combined_df, out_file)
