...
# TO DO have to add coloc plots for SYN and BSN
# add % plot for both metrics over time
```{r}
# ------------------------------------------------------------------- #
# Synatic marker evaluation iN and iDA, day 30/38 in Ctrl and ASAH1 
...
make_syn_barplots_with_stats <- function(df_input,
  ...
  # 3.2 All 4-group pairwise comparisons (per day plots)
  ...
  for (metric_var_name in metrics_keep) {
    ...
    for (this_day in c(30L, 38L)) {
      ...
      # 3.3.1 Pairwise Wilcoxon tests, BH correction, symbols
      ...
      # 3.3.2 Positioning of significance bars
      ...
      # 3.3.3. Plot: mean ± SEM, jittered raw data, significance brackets
    ...
    # 3.4 ---- Merged-days plot (8 groups)
    # 3.4.1 prepare df for all 8 result groups for plotting. Summarize & calculate sem for plotting
    df_merged <- df_clean %>%
    ...
    # 3.4.2 build y-positions for significance brackets (per stain)
    ...
    # 3.4.3 Plot with legend at bottom + stats brackets
