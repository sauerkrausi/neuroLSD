...
make_syn_barplots_with_stats <- function(df_input,
  ...
  group_levels <- levels(df_clean$group_id)
  all_comparisons <- combn(group_levels, 2, simplify = FALSE)

  # collector for all stats (to save as CSV)
  stats_all_list <- list()
    ...
    for (this_day in c(30L, 38L)) {
      ...
      stats_by_stain <- df_day %>%
        ...
        map_df(function(df_stain) {
          ...
          # BH adjust within this stain
          pair_rows %>%
            mutate(p_adj = p.adjust(p_raw, method = "BH"))
        }) %>%
        mutate(
          metric = metric_var_name,
          day = this_day,
          p_symbol = .p_to_symbol(p_adj)
        )

      # store stats to global list
      stats_all_list[[paste(metric_var_name, this_day, sep = "_")]] <- stats_by_stain

      # build data for ggpubr::stat_pvalue_manual
      # need: y.position, xmin, xmax, label, and facetting keys (stain_name)
      # We place comparisons for each stain above its max jitter/bars, staggered
      y_positions_df <- df_summary %>%
        group_by(stain_name) %>%
        summarise(base_y = max(ymax, na.rm = TRUE), .groups = "drop") %>%
        mutate(base_y = ifelse(is.finite(base_y), base_y, 0))

      # how many comparisons per stain?
      n_cmp <- length(all_comparisons)
      # build a per-stain sequence of y positions
      pval_annot <- stats_by_stain %>%
        left_join(y_positions_df, by = "stain_name") %>%
        group_by(stain_name) %>%
        mutate(
          # stagger by 5% of base_y (fallback to small constant if base_y is zero)
          step = ifelse(base_y > 0, 0.05 * base_y, 0.05),
          y.position = base_y + seq_len(dplyr::n()) * step,
          xmin = group1,
          xmax = group2,
          label = p_symbol
        ) %>%
      ...
      plot_fname <- file.path(
        output_dir,
        paste0("syn_barplot_", metric_var_name, "_day", this_day, ".pdf")
      )
      ggsave(plot_fname, p_syn, width = 7.5, height = 2.2, units = "in", device = cairo_pdf)

      # also handy PNG
      png_fname <- file.path(
        output_dir,
        paste0("syn_barplot_", metric_var_name, "_day", this_day, ".png")
      )
      ggsave(png_fname, p_syn, width = 7.5, height = 4.2, units = "in", dpi = 300)
    }
  }
  ...
  stats_csv <- file.path(output_dir, "syn_barplot_pairwise_stats_BH.csv")
  write_csv(stats_all_df, stats_csv)

  invisible(list(plots_stats_csv = stats_csv, stats = stats_all_df))
}
