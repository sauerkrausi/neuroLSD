...
# For SYP object_count, exclude empty image frames (keep >= 500)
if (metric_var_name == "object_count") {
  df_metric <- df_metric %>%
    filter(stain_norm != "SYP" | .data[[metric_var_name]] >= 500)
}
    ...
    for (this_day in c(30L,38L)) {
      ...
      for (fg in unique(df_day$facet_group)) {
        ...
        if (metric_var_name %in% coloc_metrics) {
          ...
          # source data export
          write_csv(df_day_fg %>% select(group_id, genotype, neuron_type, day, all_of(metric_var_name)), file.path(output_dir, paste0("sourcedata_syn_violin_",metric_var_name,"_day",this_day,"_",fg,".csv")))

          # 3.3.3 Extra: Day 38 coloc only: Ctrl_iN, Ctrl_iDA, ASAH1_iDA (no ASAH1_iN)
          if (this_day==38L) {
            groups_3 <- c("Ctrl_iN","Ctrl_iDA","ASAH1_iDA")
            df_3grp <- df_day_fg %>%
              filter(group_id %in% groups_3) %>%
              mutate(group_id=factor(group_id, levels=groups_3))
            if(nrow(df_3grp)>0){
              all_comparisons_3 <- combn(groups_3, 2, simplify=FALSE)
              stats_3grp <- purrr::map_df(all_comparisons_3, function(cmp) {
                g1 <- cmp[1]; g2 <- cmp[2]
                x1 <- df_3grp %>% filter(group_id==g1) %>% pull(.data[[metric_var_name]])
                x2 <- df_3grp %>% filter(group_id==g2) %>% pull(.data[[metric_var_name]])
                wt <- tryCatch(wilcox.test(x1,x2,exact=FALSE), error=function(e) list(p.value=NA_real_))
                tibble(group1=g1,group2=g2,n1=sum(is.finite(x1)),n2=sum(is.finite(x2)),p_raw=as.numeric(wt$p.value))
              }) %>% mutate(p_adj=p.adjust(p_raw,method="BH"), p_symbol=.p_to_symbol(p_adj))

              base_y_3 <- max(df_3grp[[metric_var_name]], na.rm=TRUE)
              if(!is.finite(base_y_3)) base_y_3 <- 0
              pval_3grp <- stats_3grp %>%
                mutate(step=ifelse(base_y_3>0,0.05*base_y_3,0.05),
                       y.position=base_y_3+seq_len(n())*step,
                       xmin=group1,xmax=group2,label=p_symbol) %>%
                filter(!label %in% c("ns",""))

              p_3grp <- ggplot(df_3grp,aes(x=group_id,y=.data[[metric_var_name]],fill=group_id)) +
                geom_jitter(width=0.15,height=0,alpha=0.2,color="black") +
                geom_violin(width=0.85,scale="width",trim=TRUE) +
                geom_boxplot(width=0.25,outlier.shape=NA,fill="white",color="black",fatten=1) +
                scale_fill_manual(values=group_colors,guide="none") +
                labs(x="Genotype × Neuron type", y=y_lab,
                     title=paste0("Day 38 · ",metric_var_name," · ",fg," · 3 groups")) +
                theme_bw(base_size=6)+
                theme(panel.grid=element_blank(),
                      axis.text.x=element_text(angle=30,hjust=1,vjust=1),
                      plot.title=element_text(face="bold"))
              ...
              if(nrow(pval_3grp)>0){
                p_3grp <- p_3grp + stat_pvalue_manual(pval_3grp,label="label",
                                                      xmin="xmin",xmax="xmax",
                                                      y.position="y.position",
                                                      tip.length=0.01,bracket.size=0.3,size=2.0) +
                  expand_limits(y=max(pval_3grp$y.position,na.rm=TRUE)*1.02)
              }

              ggsave(file.path(output_dir,
                               paste0("syn_violin_",metric_var_name,"_day38_",fg,"_3grp_pvals.pdf")),
                     p_3grp,width=2.5,height=4,units="in",device=cairo_pdf)

              # source data export for 3-group plot
              write_csv(
                df_3grp %>% select(group_id, genotype, neuron_type, day, all_of(metric_var_name)),
                file.path(output_dir, paste0("sourcedata_syn_violin_",metric_var_name,"_day38_",fg,"_3grp.csv"))
              )
            }
          }
          ...
          # source data export for bar plots
          write_csv(
            df_day_fg %>% select(group_id, genotype, neuron_type, day, stain_norm, all_of(metric_var_name)),
            file.path(output_dir, paste0("sourcedata_syn_bar_",metric_var_name,"_day",this_day,"_",fg,".csv"))
          )
        }
      }
    }
