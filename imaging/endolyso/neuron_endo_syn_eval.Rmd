    ...
    for (this_day in c(30L,38L)) {
      ...
      for (fg in unique(df_day$facet_group)) {
        ...
        if (metric_var_name %in% coloc_metrics) {
          ...
          # source data export
          write_csv(
            df_day_fg %>% select(group_id, genotype, neuron_type, day, all_of(metric_var_name)),
            file.path(output_dir, paste0("sourcedata_syn_violin_",metric_var_name,"_day",this_day,"_",fg,".csv"))
          )
