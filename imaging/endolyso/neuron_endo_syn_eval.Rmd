...
make_syn_barplots_with_stats <- function(df_input,
  ...
  for (metric_var_name in metrics_keep) {
    df_clean <- df_base %>% filter(is.finite(.data[[metric_var_name]]))
    df_clean <- df_clean %>%
      mutate(facet_group = ifelse(metric_var_name %in% coloc_metrics,
                                  "SYP-Bassoon", as.character(stain_name)))
    ...
    df_merged <- df_clean %>%
      mutate(facet_group=ifelse(metric_var_name %in% coloc_metrics,"SYP-Bassoon",as.character(stain_name)),
