...
```{r}
# --- 7. Group summary and plotting with jittered per-repeat points
...
invisible(
  split(ogb5n_group_summary, ogb5n_group_summary$Measurement) %>%
    imap(function(df_bar, meas_name) {

      df_pts <- ogb5n_rep_summary %>%
        filter(Measurement == meas_name) %>%
        mutate(Group = factor(Group, levels = levels(df_bar$Group)))

      p <- ggplot(df_bar, aes(x = Group, y = group_mean, fill = Group)) +
        geom_col(width = 0.7) + 
      ...
      ggsave(
        ...
        plot = p, width = 4, height = 4, device = "pdf"
      )
    })
)
...
# --- 8. Quick table check
ogb5n_group_summary %>%
  select(Measurement, Group, n_repeats, group_mean) %>%
  arrange(Measurement, Group) %>%
  print(n = 40)

write_csv(ogb5n_group_summary, file.path(out_dir_ogb5n, "OGB5N_stats_summary.csv"))
```
