...
outdir <- "/Users/felix/Documents/PostDoc/Harvard/03_LSD-PD/Microscopy/20250121_HeLa-Ctrl-ASAH1_TFN-488_uptakeAssay/pyEval"
...
# --- 4b. Outlier removal (IQR rule) per treatment group
...
# Apply outlier removal separately for each metric
coloc_df_clean <- coloc_df %>%
  remove_outliers_iqr("frac_coloc_eea1") %>%
  remove_outliers_iqr("frac_coloc_tfn")



# --- 5. Wilcoxon tests: Control vs ASAH1 within each treatment
# EEA1
stats_tbl <- coloc_df_clean %>%
  filter(!is.na(frac_coloc_eea1), !is.na(genotype), !is.na(treatment)) %>%
  group_by(treatment) %>%
  rstatix::wilcox_test(frac_coloc_eea1 ~ genotype) %>%
  ungroup() %>%
  mutate(
...
# TFN
stats_tbl_tfn <- coloc_df_clean %>%
  filter(!is.na(frac_coloc_tfn), !is.na(genotype), !is.na(treatment)) %>%
  group_by(treatment) %>%
  rstatix::wilcox_test(frac_coloc_tfn ~ genotype) %>%
  ungroup() %>%
  mutate(
    p_adj = p.adjust(p, method = "BH"),
    p_stars = case_when(
...
# --- 5b. y-positions for labels and x positions above ASAH1 for each treatment
y_pos_tbl_eea1 <- coloc_df %>%
  group_by(treatment) %>%
  summarise(y_pos = max(frac_coloc_eea1, na.rm = TRUE) * 1.05, .groups = "drop")

y_pos_tbl_tfn <- coloc_df %>%
  group_by(treatment) %>%
  summarise(y_pos = max(frac_coloc_tfn,  na.rm = TRUE) * 1.05, .groups = "drop")

levels_gt <- levels(coloc_df$genotype_treatment)

stats_for_plot <- stats_tbl %>%
  left_join(y_pos_tbl_eea1, by = "treatment") %>%
  mutate(
...
stats_for_plot_tfn <- stats_tbl_tfn %>%
  left_join(y_pos_tbl_tfn, by = "treatment") %>%
  mutate(
    x_label = paste0("ASAH1_", treatment),
    x_pos   = match(x_label, levels_gt),
    p_text  = paste0("BH p: ", signif(p_adj, 3), "  ", p_stars)
...
# --- 6a. Plot EEA1 (violin + box + jitter) with p labels
plot_frac_coloc_eea1 <- ggplot(coloc_df_clean, aes(x = genotype_treatment, y = frac_coloc_eea1, fill = genotype)) +
...
# --- 6b. Plot TFN (violin + box + jitter) with p labels
plot_frac_coloc_tfn <- ggplot(coloc_df_clean, aes(x = genotype_treatment, y = frac_coloc_tfn, fill = genotype)) +
  geom_jitter(width = 0.1, size = 1, alpha = 0.6) +
  geom_violin(trim = TRUE, linewidth = 0.2, alpha = 0.6) +
  geom_boxplot(width = 0.2, outlier.shape = NA, alpha = 0.85, fill = "white") +
  scale_fill_manual(values = hela_genotype_colors) +
  labs(x = "Genotype + Treatment", y = "Fraction of TFN colocalized") +
  theme_bw() +
  theme(panel.grid = element_blank(), legend.position = "none") +
  geom_text(
    data = stats_for_plot_tfn,
    aes(x = x_pos, y = y_pos, label = p_text),
    inherit.aes = FALSE,
    size = 3
  ) +
...
print(plot_frac_coloc_tfn)
ggsave(file.path(outdir, "HeLa_EEA1-TFN_coloc_tfn.pdf"), plot_frac_coloc_tfn, width = 3, height = 5, device = cairo_pdf)

# --- 7. Write annotated CSVs with BOTH stats merged by treatment
coloc_df %>%
  left_join(stats_tbl %>% select(treatment, p, p_adj, p_stars) %>%
              rename(p_eea1 = p, p_adj_eea1 = p_adj, p_stars_eea1 = p_stars),
            by = "treatment") %>%
  left_join(stats_tbl_tfn %>% select(treatment, p, p_adj, p_stars) %>%
              rename(p_tfn = p, p_adj_tfn = p_adj, p_stars_tfn = p_stars),
            by = "treatment") %>%
  readr::write_csv(file.path(outdir, "pairwise_colocalization_annotated.csv"))
...
summary_df %>%
  ...
  left_join(stats_tbl_tfn %>% select(treatment, p, p_adj, p_stars) %>%
              rename(p_tfn = p, p_adj_tfn = p_adj, p_stars_tfn = p_stars),
            by = "treatment") %>%
  readr::write_csv(file.path(outdir, "per_image_summary_annotated.csv"))

# --- 8. Save each stats table separately as well
readr::write_csv(stats_tbl,      file.path(outdir, "EEA1_coloc_wilcoxon_by_treatment.csv"))
readr::write_csv(stats_tbl_tfn,  file.path(outdir, "TFN_coloc_wilcoxon_by_treatment.csv"))
...
hela_base_dir <- "/Users/felix/Documents/PostDoc/Harvard/03_LSD-PD/Microscopy/20250613_HeLa-Ctrl-ASAH1_EndoLysoStains/cpsamEL/eval"
