...
hela_plot_coloc_metric <- function(input_df, y_col, y_lab, out_name, palette_vec) {
  ...
  ggsave(file.path(hela_out_dir, out_name), plot_color_metric, width = 8, height = 5)
}
...
hela_plot_summary_metrics <- function(input_df, metric_vec, title_txt, out_name, palette_vec) {
  ...
  cleaned_list <- lapply(metric_vec, function(mn) {
    ...
    cl %>% dplyr::select(genotype, !!rlang::sym(mn)) %>% dplyr::mutate(metric = mn) %>%
  ...
  plot_summary_dummy <- ggplot(long_df, aes(x = genotype, y = value, fill = genotype)) +
    geom_violin(trim = FALSE, linewidth = 0.2) +
    ...
    theme(panel.grid = element_blank(), legend.position = "none",
                   strip.background = element_rect(fill = "grey95", color = "grey80"))
  ggsave(file.path(hela_out_dir, out_name), plot_summary_dummy, width = 8, height = 5)
}
