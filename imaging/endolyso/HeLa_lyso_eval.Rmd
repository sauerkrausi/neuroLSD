...
# Helper to plot any colocalization metric by pair with stats
hela_plot_coloc_metric <- function(input_df, y_col, y_lab, out_name, palette_vec) {
  ...
  stats_tbl <- input_df %>%
    ...
    group_by(pair) %>%
    do({
      if (n_distinct(.$genotype) < 2) tibble(statistic = NA_real_, p = NA_real_)
      else {
...
hela_plot_coloc_metric(
  ...
  "Fraction coloc in A", "HeLa_frac_coloc_A_by_genotype.pdf",
  hela_genotype_colors
)

hela_plot_coloc_metric(
  HeLa_colocalization_df, "frac_coloc_B",
  "Fraction coloc in B", "HeLa_frac_coloc_B_by_genotype.pdf",
  hela_genotype_colors
)

# If you also want the original coloc_per_nucleus with the fixed order, re-run:
hela_plot_coloc_metric(
  HeLa_colocalization_df, "coloc_per_nucleus",
  "Colocalization per nucleus", "HeLa_coloc_per_nucleus_by_genotype.pdf",
  hela_genotype_colors
)

# ---------------- per_image_summary plots ----------------
...
# Helper to plot a set of summary metrics together as facets
hela_plot_summary_metrics <- function(input_df, metric_vec, title_txt, out_name, palette_vec) {
  ...
  long_df <- input_df %>%
    select(genotype, all_of(metric_vec)) %>%
    pivot_longer(cols = all_of(metric_vec), names_to = "metric", values_to = "value")
