...
hela_metric_specs <- tibble::tibble(
  metric = c("coloc_per_nucleus","frac_coloc_A","frac_coloc_B",
             "avg_area_HA","avg_area_EEA1","avg_area_VPS35",
             "HA_per_nucleus","EEA1_per_nucleus","VPS35_per_nucleus"),
  clamp01 = c(FALSE,TRUE,TRUE, FALSE,FALSE,FALSE, FALSE,FALSE,FALSE),
  nonneg  = c(TRUE,TRUE,TRUE, TRUE,TRUE,TRUE, TRUE,TRUE,TRUE)
)

hela_flag_outliers <- function(df, group_vars, value_col, method="mad", k_mad=3){
  spec <- hela_metric_specs %>% filter(metric == value_col)
  clamp01 <- ifelse(nrow(spec)>0, spec$clamp01, FALSE)
  nonneg  <- ifelse(nrow(spec)>0, spec$nonneg,  FALSE)
  df %>% mutate(.val = .data[[value_col]]) %>%
    mutate(.finite = is.finite(.val),
           .inrange = ifelse(clamp01, (.val >= 0 & .val <= 1), TRUE),
           .inrange = ifelse(nonneg, .inrange & (.val >= 0), .inrange)) %>%
    mutate(.keep_basic = .finite & .inrange) %>%
    group_by(across(all_of(group_vars))) %>%
    mutate(.med = median(.val[.keep_basic], na.rm=TRUE),
           .mad = mad(.val[.keep_basic], constant=1.4826, na.rm=TRUE),
           .low = .med - k_mad*.mad,
           .high = .med + k_mad*.mad) %>%
    ungroup() %>%
    mutate(.outlier_num = .keep_basic & (.val < .low | .val > .high),
...
for (m in coloc_metrics_vec) {
  ...
  readr::write_csv(flagged %>% filter(.drop), file.path(hela_out_dir, paste0("REMOVED_",m,".csv")))
  HeLa_colocalization_df <- flagged %>% filter(!.drop) %>% select(-starts_with(".")) %>% as.data.frame()
}

# clean summary metrics
summary_metrics_all <- c("avg_area_HA","avg_area_EEA1","avg_area_VPS35",
                         "HA_per_nucleus","EEA1_per_nucleus","VPS35_per_nucleus")
for (m in summary_metrics_all) {
  if (!m %in% names(HeLa_summary_df)) next
  flagged <- hela_flag_outliers(HeLa_summary_df, c("genotype"), m)
  readr::write_csv(flagged %>% filter(.drop), file.path(hela_out_dir, paste0("REMOVED_",m,".csv")))
  HeLa_summary_df <- flagged %>% filter(!.drop) %>% select(-starts_with(".")) %>% as.data.frame()
}
...
hela_plot_coloc_metric <- function(input_df, y_col, y_lab, out_name, palette_vec) {
  ...
  stats_for_plot <- left_join(summary_stats_table, y_pos_tbl, by = "pair")

  plotting_object_dummy <- ggplot(input_df, aes(x = genotype, y = .data[[y_col]], fill = genotype)) +
  ...
  ggsave(file.path(hela_out_dir, out_name), plotting_object_dummy, width = 6, height = 5)
}
...
hela_plot_summary_metrics <- function(input_df, metric_vec, title_txt, out_name, palette_vec) {
  ...
  plotting_object_summary <- ggplot(long_df, aes(x = genotype, y = value, fill = genotype)) +
    geom_violin(trim = FALSE, linewidth = 0.2) +
  ...
  ggsave(file.path(hela_out_dir, out_name), plotting_object_summary, width = 10, height = 7)
}
